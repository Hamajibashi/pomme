        ...
        <div class="py-5">
            ...
            <div class="bg-primary text-secondary p-5 text-center mb-3" v-else>
                ...
                <div class="d-flex flex-column flex-md-row justify-content-center">
                    <router-link to="/menu"
                        class="btn btn-outline-secondary text-light mx-2 mb-2 mb-md-0">看看菜單</router-link>
                        ...
                        <table class="table mt-4 text-primary">
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td class="border-0 text-end text-break" v-if="order.message">{{ order.message }}</td>
                                    <td class="text-end border-0 text-muted" v-else>（空白）</td>
                                </tr>
                            </tbody>
                        </table>
                        <button class="btn btn-lg btn-primary w-100" @click="payOrder" :disabled="order.is_paid">
                            付款
                        </button>
                    </div>
                </div>
                            ...
                            <tr class="border-bottom border-top" v-for="item in order.products" :key="item.id">
                                ...
                                <th scope="row" class="border-0 px-0 py-4 d-flex align-items-center">
                                    <img :src="item.product.imageUrl" :alt="item.product.title"
                                        class="object-cover img-fluid" style="width: 72px; height: 72px;">
                                    <div class="ms-3">
                                ...
                                <!-- 商品價格 -->
                                <td class="border-0 align-middle d-none d-md-table-cell">
                                    <p class="mb-0 pe-5">NT$ {{ item.product.price }}</p>
                                </td>
                                <!-- 商品總額 -->
                                <td class="border-0 align-middle d-none d-md-table-cell text-end">
                                    <p class="mb-0 pe-2">NT$ {{ item.total }}</p>
                                </td>
                            </tr>
                        </tbody>
                        <tfoot>
                            <tr class="text-end fw-bold">
                                <td colspan="2">總金額</td>
                                <td colspan="2" class="fs-3">$ {{ order.total }}</td>
                            </tr>
                        </tfoot>
                    </table>
                </div>
