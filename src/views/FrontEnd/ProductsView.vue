        ...
        <div class="col-md-9" data-aos="fade-up" data-aos-duration="800">
          <div class="row">
            <div class="col-md-6 col-lg-4 mb-3" v-for="product in products" :key="product.id">
              <div class="card border-0 shadow-sm h-100 cursor-pointer" @click="$router.push(`/product/${product.id}`)">
                <img :src="product.imageUrl" class="card-img-top object-cover img-hover" :alt="product.title"
                  height="300">
                <div class="card-body p-4 d-flex justify-content-between align-items-center">
                  <div>
                    ...
                    <p class="card-text mb-0 h5">NT$ {{ product.price }} <span class="text-muted fs-6"
                        v-if="product.price !== product.origin_price"><del>NT${{ product.origin_price }}</del></span>
                    </p>
                  </div>
